...
A searchable dropdown with grouped options. Modeled on shadcnblocks **combobox-grouped-6**, restyled with the starter's design tokens. Live filtering, full keyboard navigation, group headings that hide automatically when their items are filtered out, an empty state, and form integration via a hidden input.
